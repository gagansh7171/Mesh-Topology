		...
		Node(){
			...
			if( input[0] == 'Y' || input[0] == 'y' ){
                ...
                while(true){
                    cout<<"Enter Message : ";
                    char message[2000]; 
                    strcpy(message, ("Hello from device" + to_string(device_id) + " msg_" + to_string(_) ).c_str()  );
                    _++;
                    int x = rand()%(NUMDEVICES-1) + 1;
                    cout<<message<<endl;
                    cout<<"Enter number of recepients : "<<x<<endl;
                    
                    cout<<"Enter space separated Recepients : ";

                    int reciepents[x];
                    for(int i = 0; i<x; i++){
                        reciepents[i] = rand()%(NUMDEVICES-1) + 1;
                        cout<<reciepents[i]<<" ";
                    }
                    cout<<endl;
